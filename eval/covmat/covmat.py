# This code is verbatim from the GeoDiff codebase
import torch
import numpy as np
import pandas as pd
...
# Note by aniketh -- I think this computes the pairwise distances...
...
# Note by aniketh -- I think this computes the final COVMAT scores
...
# @aniketh -- You need to address the packed input
class CovMatEvaluator(object):
    ...
    def __call__(self, packed_data_list, start_idx=0):
        ...
        for data in packed_data_list:
            ...
            # Note from @aniketh (I dont think these reshapes should be an issue)
            data['pos_ref'] = data['pos_ref'].reshape(-1, data['rdmol'].GetNumAtoms(), 3)
            data['pos_gen'] = data['pos_gen'].reshape(-1, data['rdmol'].GetNumAtoms(), 3)

            # Note from @aniketh (I dont think this part should be an issue)
