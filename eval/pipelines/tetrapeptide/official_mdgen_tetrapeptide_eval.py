...
def main(peptide_name):
    # print(f'\nThe evaluated peptide is {peptide_name}')
    out = {}
    
    # Check if we have this peptide in the mdgen data
    if peptide_name not in peptide_confs:
        print(f"No reference data found for peptide {peptide_name}")
        return peptide_name, {}
    
    # Get RDKit molecule and SMILES
    mol = peptide_confs[peptide_name]['rdkit_mol']
    smiles = peptide_confs[peptide_name].get('smiles', 'UNKNOWN')
    
    # Load reference trajectory from preprocessed cache
    peptide_dir = os.path.join(args.mdgen_data_dir, peptide_name)
    cache_file = os.path.join(peptide_dir, f'{peptide_name}_cache_stride100.npz')
    pdb_file = os.path.join(peptide_dir, f'{peptide_name}.pdb')
    
    if not os.path.exists(cache_file):
        print(f"Missing cache file for {peptide_name}: {cache_file}")
        print(f"Please run: python data_gen/preprocess/process_mdgen.py --stride 100 --num_workers 32")
        return peptide_name, {}
    
    if not os.path.exists(pdb_file):
        print(f"Missing PDB file for {peptide_name}")
        return peptide_name, {}
    
    try:
        # Load preprocessed coordinates from cache (already H-removed, centered, aligned, in Angstroms)
        cached = np.load(cache_file)
        xyz_cached = cached['xyz']  # Shape: (T, N, 3) in Angstroms
        
        # Load topology from PDB
        pdb_traj = md.load_pdb(pdb_file)
        
        # Remove hydrogens from topology to match cached coordinates
        mol_noH = Chem.RemoveHs(mol)
        z_full = [atom.GetAtomicNum() for atom in mol.GetAtoms()]
        keep_idxs = list(mol.GetSubstructMatch(mol_noH))
        pdb_traj = pdb_traj.atom_slice(keep_idxs)
        mol = mol_noH
        
        # Create MDTraj trajectory object with cached coordinates and topology
        # Cache is in Angstroms, MDTraj expects nm, so convert back
        ref_traj_full = md.Trajectory(xyz_cached / 10.0, pdb_traj.topology)
        
    except Exception as e:
        print(f"Error loading cached trajectory for {peptide_name}: {e}")
        return peptide_name, {}
    
    name = peptide_name  # Use peptide name (e.g., "ITKD")
    
    ### Read pickle file from gener_dir
    # Check if we have generated data for this peptide
    if smiles not in gener_data:
        print(f"No generated data for peptide {peptide_name} (SMILES: {smiles})")
        return peptide_name, {}
    ...
    gener_traj_coords = gener_data[smiles]['coords']  # (5, N, 3, T)
    
    # Check for NaN values in generated coordinates
    for i in range(len(gener_traj_coords)):
        if np.isnan(gener_traj_coords[i]).any():
            print(f"WARNING: NaN encountered in generated traj at index {i} for peptide {peptide_name}, skipping")
            return peptide_name, {}
    
    if args.check_nan:
        # Early return for NaN checking mode
        return peptide_name, out
    ...
    ### Reference trajectory is already downsampled (loaded from cache with stride=100)
    ref_traj_downsampled = ref_traj_full
    ...
    if any(np.isnan(feat).any() for feat in gener_features):
        print(f"WARNING: NaN values detected in generated features for {peptide_name}, skipping this peptide")
        return peptide_name, {}
    
    if any(np.isnan(feat).any() for feat in gener_features_cossin):
        print(f"WARNING: NaN values detected in generated cos/sin features for {peptide_name}, skipping this peptide")
        return peptide_name, {}
    
    # Check for empty feature arrays
    if ref_features.shape[1] == 0 or any(feat.shape[1] == 0 for feat in gener_features):
        print(f"WARNING: Empty feature arrays for {peptide_name}, skipping this peptide")
        return peptide_name, {}
    ...
    if not args.no_msm:
        ...
        try:
            ...
            print('ERROR', e, peptide_name, flush=True)

    return peptide_name, out

parser = argparse.ArgumentParser()
parser.add_argument('--time_step', type=float, default=10.0)  # 10 ps frame rate for tetrapeptides
parser.add_argument('--mdgen_data_dir', type=str, default=os.environ.get('MDGEN_DATA_ROOT', '') + '/data/4AA_sims',
                    help='Path to MDGen raw trajectory data directory')
parser.add_argument('--mdgen_pickle', type=str, default='data_gen/tetrapeptide_conformers_mdgen.pkl',
                    help='Path to MDGen RDKit molecules pickle')
parser.add_argument('--test_split', type=str, default=os.environ.get('MDGEN_DATA_ROOT', '') + '/splits/4AA_test.csv',
                    help='Path to test split CSV')
parser.add_argument('--gener_dir', type=str, default='model_outputs_official/mdgen_trajectory_official/mdgen_interpolator_test_generations.pkl')
...
print('Loading MDGen RDKit molecules pickle...')
with open(args.mdgen_pickle, 'rb') as f:
    peptide_confs = pickle.load(f)
print(f'Loaded {len(peptide_confs)} peptide structures')

# Load test split CSV
print('Loading test split...')
test_df = pd.read_csv(args.test_split)
test_peptides = set(test_df['name'].str.strip())
print(f'Test split has {len(test_peptides)} peptides')
...
# Make gener_data globally accessible for main()
gener_data = gen_dict

# Filter out non-peptide keys (keep only SMILES strings)
gener_smiles_all = [k for k in gen_dict.keys() if k != 'WALL_CLOCK' and isinstance(k, str) and 'C' in k]
print(f'Found {len(gener_smiles_all)} generated SMILES')

# Build SMILES to peptide name mapping
print('Building SMILES to peptide name mapping...')
smiles_to_peptide = {}
for peptide_name in test_peptides:
    if peptide_name in peptide_confs:
        smiles = peptide_confs[peptide_name].get('smiles', None)
        if smiles:
            smiles_to_peptide[smiles] = peptide_name

# Filter to only evaluate peptides that have:
# 1. Generated data (by SMILES)
# 2. Are in test split
# 3. Have preprocessed cache files
peptides_to_eval = []
for smiles in gener_smiles_all:
    if smiles in smiles_to_peptide:
        peptide_name = smiles_to_peptide[smiles]
        # Check if cache file exists
        peptide_dir = os.path.join(args.mdgen_data_dir, peptide_name)
        cache_file = os.path.join(peptide_dir, f'{peptide_name}_cache_stride100.npz')
        pdb_file = os.path.join(peptide_dir, f'{peptide_name}.pdb')
        if os.path.exists(cache_file) and os.path.exists(pdb_file):
            peptides_to_eval.append(peptide_name)

print(f'Found {len(peptides_to_eval)} peptides with both generated and reference data')
print(f'Skipped {len(gener_smiles_all) - len(peptides_to_eval)} peptides without complete data')
...
if args.num_workers > 1:
    ...
    __map__ = map
out = dict(tqdm.tqdm(__map__(main, peptides_to_eval), total=len(peptides_to_eval)))
if args.num_workers > 1:
