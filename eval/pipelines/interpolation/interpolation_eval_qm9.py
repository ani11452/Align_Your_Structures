...
def main(smiles):
    ...
    for i in range(len(gener_traj_tensor)):
        ...
        for j in range(torsions.shape[1]):
            ...
            assert torsions_cossin.shape == (101, 2), f'The torsion should have shape (500, 2), but it has shape {torsions_cossin.shape}'
            torsions_2dim.append(torsions_cossin)
        torsions_2dim = np.concatenate(torsions_2dim,axis=1)
        assert torsions_2dim.shape[0] == 101, f'The torsion should have shape (500, 2N), but it has shape {torsions_2dim.shape}'
    ...
    ref_coarse_traj = msm.metastable_assignments[ref_clusters]   # A list of array: (500,)
    ...
    assert len(ref_coarse_traj) == 120, f'The coarse traj should have length 120, but it has length {len(ref_coarse_traj)}'
    assert ref_coarse_traj[0].shape == (500,), f'The coarse traj should have shape (500, ), but it has shape {ref_coarse_traj[0].shape}'
...
parser.add_argument('--gener_dir', type=str, default='model_outputs_official/qm9_interpolation/qm9_noH_1000_kabsch_traj_interpolator_pretrain_539_25_inter/epoch=299-step=13200-gen.pkl')
...
parser.add_argument('--save_name', type=str, default='qm9_interpolation_evalout')
