...
def main(smiles):
    ...
    if name != 'O_C_O_c1_nH_c2ccc_Cl_cc2c1CC_O_N1CCN_c2ccccc2_CC1_20':
        return smiles, {}
    ...
    with open(args.gener_dir, 'rb') as f:
        ...
        gener_traj_coords = gener_data[smiles]['coords']  # (5, 9, 3, 500), 10 trajs of 500 length.
        print(len(gener_traj_coords))
    assert gener_traj_coords[0].shape[2] == 1001, f'The generated coords should have shape (N, 3, 1001), but it has shape {gener_traj_coords[0].shape}'
    ...
    assert gener_egtn_traj_coords[0].shape[2] == 1001, f'The generated coords should have shape (N, 3, 500), but it has shape {gener_egtn_traj_coords[0].shape}'
    ...
    # In each traj, sample 5 down-sampled trajs. Then we have 4 * 5 in total. The new list should have shape (960, N) with length 20.
    downsampled_ref_torsion = []
    for traj in ref_torsion_tensor:
        assert traj.shape[0] == 12500, f'The ref torsion should have shape (12500, N), but it has shape {traj.shape}, the molecule is {smiles}'
    ...
    gener_traj_tensor = []
    for i in range(len(gener_traj_coords)):
        gener_traj_tensor.append(torch.permute(torch.from_numpy(gener_traj_coords[i] / 10), (2,0,1))[:960])      # /10 convert to nm
    # assert len(gener_traj_tensor) == 10, f'The gener traj should have length 10, but it has length {len(gener_traj_tensor)}'
    assert gener_traj_tensor[0].shape[0] == 960, f'The gener traj should have shape (960, N, 3), but it has shape {gener_traj_tensor[0].shape}'
    assert gener_traj_tensor[0].shape[2] == 3, f'The gener traj should have shape (960, N, 3), but it has shape {gener_traj_tensor[0].shape}'
    
    # Load gener egtn traj
    gener_egtn_traj_tensor = []
    for i in range(len(gener_egtn_traj_coords)):
        gener_egtn_traj_tensor.append(torch.permute(torch.from_numpy(gener_egtn_traj_coords[i] / 10), (2,0,1))[:960])      # /10 convert to nm
    # assert len(gener_traj_tensor) == 10, f'The gener traj should have length 10, but it has length {len(gener_traj_tensor)}'
    assert gener_egtn_traj_tensor[0].shape[0] == 960, f'The gener traj should have shape (960, N, 3), but it has shape {gener_egtn_traj_tensor[0].shape}'
    assert gener_egtn_traj_tensor[0].shape[2] == 3, f'The gener traj should have shape (960, N, 3), but it has shape {gener_egtn_traj_tensor[0].shape}'
    ...
    if args.plot:
        ...
        pyemma.plots.plot_feature_histograms(np.concatenate(gener_torsion_tensor), feature_labels=feat_dihedral, ax=ax, color='green')
    ...
    assert gener_bond_angles_tensor[0].shape[0] == 960, f'The gener bond angles should have shape (10*500, N), but it has shape {gener_bond_angles_tensor.shape}'
    ...
    if args.plot:
        ...
        pyemma.plots.plot_feature_histograms(md_bond_angles[:,:10], ax=ax, color='blue', label='MD')
        pyemma.plots.plot_feature_histograms(np.concatenate(gener_bond_angles_tensor, axis=0)[:,:10], ax=ax, color='green', label='Generated')
        ax.set_ylabel('Feature No.')
    ...
    assert gener_bond_lengths_tensor[0].shape[0] == 960, f'The gener bond lengths should have shape (10*500, N), but it has shape {gener_bond_lengths_tensor.shape}'
    ...
    if args.plot:
        ...
        pyemma.plots.plot_feature_histograms(np.concatenate(gener_bond_lengths_tensor,axis=0)[:, :10], ax=ax, color='green', label='Generated')
    ...
    for i, feat in enumerate(feat_dihedral.describe()):
        ...
        for j in range(len(ref_torsion_tensor)):  # Calculate for each torsion angle in each ref trajectory
            ...
            autocorr = acovf(np.sin(this_torsion), demean=False, adjusted=True, nlag=900) + acovf(np.cos(this_torsion), demean=False, adjusted=True, nlag=900)
        ...
        for j in range(len(gener_egtn_torsion_tensor)):
    ...
    if args.plot:
        ...
        fig.tight_layout()
        fig.savefig(f'{plot_dir}/{name}_torsion_autocorrelation.pdf', bbox_inches='tight')
        plt.close()
    ...
    for i in range(len(gener_traj_tensor)):
        ...
        for j in range(torsions.shape[1]):
            ...
            assert torsions_cossin.shape == (960, 2), f'The torsion should have shape (960, 2), but it has shape {torsions_cossin.shape}'
            torsions_2dim.append(torsions_cossin)
        torsions_2dim = np.concatenate(torsions_2dim,axis=1)
        assert torsions_2dim.shape[0] == 960, f'The torsion should have shape (960, 2N), but it has shape {torsions_2dim.shape}'
        assert torsions_2dim.shape[1] % 2 == 0, f'The torsion should have shape (960, 2N), but it has shape {torsions_2dim.shape}'
        gener_torsion_tensor.append(torsions_2dim)
    assert ref_torsion_tensor[0].shape == gener_torsion_tensor[0].shape, f'The shape of ref and gener torsion tensor should be both 960, but they are {ref_torsion_tensor[0].shape} and {gener_torsion_tensor[0].shape}'
    ...
    if ref_tica[0].shape[1] > 2:  # has more than 1 tica component
        ...
        if args.plot:
            fig, axs = plt.subplots(1, 3, figsize=(15, 5))
            pyemma.plots.plot_free_energy(*ref_concatenate[:, :2].T, ax=axs[0], cbar=True)
            pyemma.plots.plot_free_energy(*np.concatenate(gen_tica)[:, :2].T, ax=axs[1], cbar=True)
            pyemma.plots.plot_free_energy(*np.concatenate(md_tica)[:, :2].T, ax=axs[2], cbar=True)
...
parser.add_argument('--gener_dir', type=str, default='model_outputs_official/drugs_trajectory_official/drugs_noH_1000_kabsch_traj_interpolator_pretrain_fs_25/epoch=399-step=71200-gen_official_merged.pkl')
...
gen_egtn_dir = 'model_outputs_official/drugs_trajectory_official/drugs_noH_1000_kabsch_traj_egtn_fs/epoch=399-step=71200-gen.pkl'
