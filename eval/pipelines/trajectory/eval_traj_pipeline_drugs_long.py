...
def main(smiles):
    ...
    with open(args.gener_dir, 'rb') as f:
        ...
        gener_traj_coords = gener_data[smiles]['coords']  # Can be (N, 3, 4001) for single long traj or (num_traj, N, 3, T) for multiple trajs
    ...
    # Handle single long trajectory (N, 3, 4001) format
    gener_traj_coords = gener_traj_coords[0]
    if gener_traj_coords.ndim == 3:  # Single trajectory: (N, 3, T)
        # Convert to (T, N, 3) - keep the full 4001 frames as one continuous trajectory
        full_traj = torch.permute(torch.from_numpy(gener_traj_coords / 10), (2, 0, 1))  # /10 convert to nm
        gener_traj_tensor.append(full_traj)  # Keep as one continuous trajectory
    else:  # Old format: multiple trajectories (num_traj, N, 3, T)
        for i in range(len(gener_traj_coords)):
            # gener_traj_tensor.append(torch.permute(torch.from_numpy(gener_traj_coords[i] / 10), (2,0,1))[:960])      # /10 convert to nm
            gener_traj_tensor.append(torch.permute(torch.from_numpy(gener_traj_coords[i] / 10), (2,0,1))[0:960]) 
    ...
    assert gener_traj_tensor[0].shape[2] == 3, f'The gener traj should have shape (T, N, 3), but it has shape {gener_traj_tensor[0].shape}'
    print(f"Loaded generated trajectory with shape: {gener_traj_tensor[0].shape}")
    ...
    for i, feat in enumerate(feat_dihedral.describe()):
        ...
        for j in range(len(gener_traj_tensor)):
            ...
            autocorr = acovf(np.sin(this_torsion), demean=False, adjusted=True, nlag=240) + acovf(np.cos(this_torsion), demean=False, adjusted=True, nlag=240)
            baseline = np.sin(this_torsion).mean()**2 + np.cos(this_torsion).mean()**2
            lags = 1 + np.arange(len(autocorr))
            if 1-baseline < 1e-8:
